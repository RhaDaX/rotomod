...
class Rotomod_userswitching {
	 ...
	 * @var string
	 */
	public static $application = 'WordPress';
	 ...
	 */
	public function filter_user_row_actions( array $actions, $user ) {
		$link = self::maybe_switch_url( $user );
	...
	public static function get_instance() {
		...
		if ( ! isset( $instance ) ) {
			$instance = new Rotomod_userswitching();
		}
    ...
    public function display_link_user_switching( $menu, $args){

        $user = wp_get_current_user();
        if ( in_array( 'commercial', (array) $user->roles ) && 'main_menu' == $args->theme_location ) {
            $menu = $menu . ' <ul  class="sidebar-menu"><li id="nav-menu-item-1729" class="menu-item menu-item-type-post_type menu-item-object-page  narrow "><a href="/acces-commerciaux">Accès commerciaux</a></li></ul>';

        }
        return $menu;
    }

    public function  choose_commercial_template( $page_template )
{
    if ( is_page( 'acces-commerciaux' ) ) {
        $page_template = dirname( __FILE__ ) . '/templates/commercial-page.php';
    }
    return $page_template;
    }

    /**
     * Add capabilitie see_clients to commercial
     */
    public function add_custom_cap()
    {
        // Gets the simple_role role object.
        $role = get_role( 'commercial' );

        // Add a new capability.
        $role->add_cap( 'switch_to_user', true );
...
if ( ! function_exists( 'user_switching_set_olduser_cookie' ) ) {
	...
	function user_switching_set_olduser_cookie( $old_user_id, $pop = false, $token = '' ) {
		$secure_auth_cookie    = Rotomod_userswitching::secure_auth_cookie();
		$secure_olduser_cookie = Rotomod_userswitching::secure_olduser_cookie();
...
if ( ! function_exists( 'user_switching_clear_olduser_cookie' ) ) {
	...
	function user_switching_clear_olduser_cookie( $clear_all = true ) {
		...
		if ( $clear_all || empty( $auth_cookie ) ) {
			...
			if ( Rotomod_userswitching::secure_auth_cookie() ) {
				$scheme = 'secure_auth';
			} else {
				$scheme = 'auth';
			}
...
if ( ! function_exists( 'current_user_switched' ) ) {
	...
	function current_user_switched() {
		...
		return Rotomod_userswitching::get_old_user();
	}
}
...
$GLOBALS['user_switching'] = Rotomod_userswitching::get_instance();
